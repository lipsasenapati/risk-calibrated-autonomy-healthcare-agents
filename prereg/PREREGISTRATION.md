...
| Code commit frozen for this plan | `5e49069` |
...
| 2026-09-19 | Replaced a single hardcoded `TOKEN_PRICES_USD_PER_M` constant (`$2.00`/`$8.00` per 1M tokens, `gpt-4.1`'s price) with `token_prices_for_model()`, a lookup keyed by model snapshot prefix, applied at run time based on the actual `--model` argument. | The 16-episode pilot's provenance record showed `token_prices_usd_per_m: {input: 2.0, output: 8.0}` while running `gpt-4.1-mini-2025-04-14`, whose real price is $0.40/$1.60 per 1M tokens (verified against OpenAI's published pricing) — a 5x overstatement of the cost-per-successful-episode secondary endpoint. Fixed before the full run; all 38 tests still pass. |
| 2026-09-19 | `OpenAIResponsesAgent._request` now also retries on a bare `TimeoutError`, not only `HTTPError`/`URLError`. | The first full-benchmark attempt crashed on a 180-second read timeout mid-episode; the exception surfaced as a bare `TimeoutError` rather than being wrapped in `URLError`, so it was not caught by the existing retry logic despite the stated retry policy (Methods, "Agents and tools": "up to five retries on transient... failures"). Fixed before any arm completed a full run; all 38 tests still pass. |
